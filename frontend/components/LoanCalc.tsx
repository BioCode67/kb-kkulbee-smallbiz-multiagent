  ...
  return (
    <section className="surface-1 print:hidden overflow-hidden">
      <button onClick={() => setOpenCalc((v) => !v)}
        ...
        <div>
          <h3 className="font-display text-[16px] text-kb-ink">이 돈 빌리면 한 달에 얼마 갚을까</h3>
        ...
        <span className="text-[13px] text-kb-ink/40">{openCalc ? '접기 ▲' : '계산해 보기 ▼'}</span>
      </button>
        ...
        <div className="border-t border-kb-ink/[.07] px-5 pb-5 pt-4">
          <div className="grid gap-3 sm:grid-cols-3">
            {([['빌리는 돈 (만원)', amt, setAmt, 100],
              ...
              <label key={label} className="block">
                <span className="text-[11px] font-semibold text-kb-ink/55">{label}</span>
                <input type="number" value={val} step={step} min={0}
                  onChange={(e) => (set as (n: number) => void)(Number(e.target.value))}
                  className="mt-1 w-full rounded-lg bg-kb-ink/[.04] px-3 py-2 text-[13.5px]
          ...
          <div className="mt-3 flex gap-1.5">
            {(Object.keys(METHOD_LABEL) as Method[]).map((m) => (
              <button key={m} onClick={() => setMethod(m)}
                ...
                <span className="block text-[12px] font-bold text-kb-ink">
                  {METHOD_LABEL[m][0]}
                </span>
                <span className="block text-[10px] text-kb-ink/50">
                  {METHOD_LABEL[m][1]}
                </span>
              </button>
            ))}
          ...
          {r && (
            <div className="mt-4 grid grid-cols-3 gap-3 rounded-xl bg-kb-ink/[.04] p-4">
              <div>
                <p className="text-[10.5px] font-bold text-kb-ink/50">
              ...
              <div>
                <p className="text-[10.5px] font-bold text-kb-ink/50">총 이자</p>
          ...
          <p className="mt-3 text-[10.5px] leading-relaxed text-kb-ink/50">
