...
export default function Page() {
  ...
  // 첫인사에 실측을 담습니다 (모두봄의 '방금 계산해 봤어요' 패턴) —
  // 빈 인사말보다 "지금 마감 임박 공고 N건"이 눌러 볼 이유를 만듭니다.
  useEffect(() => {
            ...
            >
              ...
              <div className="grid w-full max-w-[1200px] grid-cols-1 items-center gap-6
                ...
                <div className="flex min-w-0 max-w-full flex-col items-center
                  ...
                  {/* 실행 마일스톤 배지 — 화면 안의 주장(실측·가드레일)이 화면
                      밖에서도 걸어온 길임을 보여줍니다. SW 출원·현장 검증·기관
                      협력. 문구는 docs/STRATEGY.md와 함께 관리합니다. */}
                  <motion.div
                    initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    {['SW 저작권 출원 완료',
                      '대구·경산 소상공인 현장 검증 완료',
                      '지역 소상공인지원센터 협업 추진 중'].map((t) => (
                      <span key={t}
                        className="inline-flex items-center gap-1.5 rounded-full
                                   border border-kb-amber/25 bg-kb-yellow/[.14] px-3
                                   py-1.5 text-[12.5px] font-semibold text-kb-amber">
