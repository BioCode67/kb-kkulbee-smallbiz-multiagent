...
# ── 자금 성격 ─────────────────────────────────────────────────────────────
#
# 사장님들이 가장 많이 헷갈리는 지점입니다. "지원사업"이라고 하면 다 주는
# 돈인 줄 아는데, 실제로는 갚아야 하는 것(융자), 이자만 대신 내주는 것
# (이차보전), 정말 주는 것(보조금), 돈이 아니라 서비스인 것(컨설팅·교육)이
# 섞여 있습니다. 5천만원 융자를 보조금으로 알고 신청했다가 상환 계획 없이
# 받는 일이 실제로 생깁니다.
#
# 그래서 공고 본문의 말로 성격을 가릅니다. 순서가 중요합니다 — '이차보전'이
# '융자'보다 먼저 걸려야 합니다. 이차보전 공고에도 '융자'가 나오기 때문입니다.
_FUNDING = [
    ("이차보전", ["이차보전", "이자차액", "이자 차액", "대출이자 지원", "이자지원",
                "이자 지원", "금융비용 지원"]),
    ("보증",     ["보증서", "보증료", "특례보증", "신용보증", "보증지원", "보증 지원"]),
    ("융자",     ["융자", "대출", "저리", "상환", "거치"]),
    ("바우처",   ["바우처", "쿠폰", "포인트 지급"]),
    ("컨설팅·교육", ["컨설팅", "교육", "멘토링", "진단", "아카데미", "역량강화",
                  "설명회", "세미나"]),
    ("보조금",   ["보조", "지원금", "지급", "무상", "실비", "구입비", "설치비"]),
]


def read_funding(title: str, support: str, summary: str) -> tuple[str, str]:
    """자금 성격과, 그것이 무슨 뜻인지 한 줄.

    **제목이 말하면 제목을 따릅니다.**

    처음에는 제목·요약·지원내용을 한 덩어리로 붙여 놓고 먼저 걸리는 것을
    골랐습니다. 그러니 「소상공인 정책자금 **융자**사업」이 '보증'이 됐습니다 —
    본문 어딘가에 '보증서'가 스쳤고 보증이 앞 순서였기 때문입니다.

    그다음엔 자리별 가중치(제목 3배)로 점수를 매겼는데, 이번에는
    「소상공인시장진흥자금 **융자** 계획 공고」가 또 '보증'이 됐습니다.
    제목에서 3점을 받아도 본문에서 보증이 여러 번 나오면 뒤집힙니다.

    제목은 작성자가 "이 사업이 무엇인가"를 압축해 적은 자리입니다. 본문의
    스치는 말이 그것을 이길 수 없어야 합니다. 그래서 제목에 자금 낱말이
    하나라도 있으면 **제목 안에서만** 고르고, 없을 때만 본문을 봅니다.
    """
    in_title = [k for k, ws in _FUNDING if any(w in (title or "") for w in ws)]
    if in_title:
        # _FUNDING 순서가 곧 우선순위입니다 — 이차보전이 융자보다 먼저여야
        # "이차보전 융자 지원"이 이차보전으로 갑니다.
        for kind, _ in _FUNDING:
            if kind in in_title:
                return kind, _FUNDING_NOTE[kind]

    best, best_score = "기타", 0
    for kind, words in _FUNDING:
        score = (2 * sum(w in (support or "") for w in words)
                 + 1 * sum(w in (summary or "") for w in words))
        if score > best_score:
            best, best_score = kind, score
    return (best, _FUNDING_NOTE[best]) if best_score >= 1 else \
           ("기타", _FUNDING_NOTE["기타"])


_FUNDING_NOTE = {
    "융자": "빌리는 돈입니다. 갚아야 하고 이자가 붙습니다.",
    "이차보전": "빌리는 것은 은행에서 하고, 이자 일부를 지자체가 대신 냅니다.",
    "보증": "담보가 부족할 때 보증기관이 대신 서 줍니다. 대출은 은행에서 실행됩니다.",
    "보조금": "갚지 않는 돈입니다. 다만 쓸 곳이 정해져 있고 정산해야 합니다.",
    "바우처": "돈 대신 정해진 서비스나 물품으로 받습니다.",
    "컨설팅·교육": "돈이 아니라 서비스입니다. 진단·교육·멘토링을 받습니다.",
    "기타": "공고 원문에서 자금 성격을 읽지 못했습니다. 원문을 확인하세요.",
}
...
def build_docs(raw: list[dict], today: date) -> list[dict]:
    ...
    for r in raw:
        ...
        # 제목·요약·지원내용을 함께 봅니다. 제목만 보면 "○○자금 지원사업"이
        # 융자인지 보조금인지 알 수 없습니다.
        funding, funding_note = read_funding(
            r.get("title", ""), " ".join(bl), summary)
