...
def build() -> str:
    ...
    # ── 7.5 자유주제 — "어떻게 했지" ─────────────────────────────────────
    s = blank(prs)
    header(s, "BEYOND", "데이터사이언스 · RPA · 생성 · 접근성 — 자유 기능 아홉")
    cols9 = [
        ("두 동네 비교", "A vs B를 같은 자로",
         "\"연남동이랑 성수동 비교해줘\" 한 마디로\n우세 판정 + 요인 6개 승부표 + 생활인구."),
        ("RPA 원문 순찰", "색인은 스냅샷, 공고는 살아 움직인다",
         "찜한 공고 전부를 기계가 원문에서 재확인 —\n마감 변동 감지, 서식 hwp, 마감은 .ics로."),
        ("민원서 초안 생성", "규칙이 법조문, LLM은 사실관계만",
         "한 문장이 제출 가능한 민원서 꼴로. 없는\n날짜·금액은 [빈칸] 강제 — §를 못 지어냄."),
        ("계약서 신호등", "지는 계약은 서명 전에 결정된다",
         "독소조항 10유형을 빨간 펜으로 짚고\n조문+서명 전 팁. 실제 문구에만 반응."),
        ("골든타임 계산기", "지는 이유는 내용이 아니라 시기",
         "청약철회 14일·해지 1년/5년·갱신요구 마감을\nD-day로. 폰 캘린더(.ics)에 알림까지."),
        ("글라스박스 콘솔", "AI가 한 일을 실측 로그로 재생",
         "라우터 근거 → 대조한 데이터 수 → 가드레일\n검사까지 노드별 실측 ms로 전 과정 공개."),
        ("닮은 상권 찾기", "업종 구성 247차원 코사인",
         "연남동 ↔ 광안리·서교동·망원동. '성격이\n닮은' 동네 — 2호점 후보의 출발점. 0.4ms."),
        ("음성 대화 + 지도 투어", "브라우저 내장 · 다운로드 0",
         "말로 묻고 꿀비가 읽어 줍니다. 전국에서 그\n동네로 날아드는 4막 투어 — 금융 접근성."),
        ("계산기 두 대", "융자의 다음 질문은 '한 달에 얼마?'",
         "공고 한도·금리 자동 입력 상환 시뮬레이터\n+ 손익분기. 확약 아님을 화면에 명시."),
    ]
    for i, (t, sub, d) in enumerate(cols9):
        x = Inches(0.75 + (i % 3) * 4.02)
        y0 = Inches(1.9 + (i // 3) * 1.78)
        rect(s, x, y0, Inches(3.72), Inches(1.64),
             fill=RGBColor(0xFA, 0xF8, 0xF4), line=LINE)
        text(s, x + Inches(0.22), y0 + Inches(0.1), Inches(3.3), Inches(0.32),
             t, size=12.5, bold=True)
        text(s, x + Inches(0.22), y0 + Inches(0.42), Inches(3.3), Inches(0.28),
             sub, size=9, bold=True, color=RGBColor(0x9A, 0x6B, 0x00))
        text(s, x + Inches(0.22), y0 + Inches(0.72), Inches(3.34), Inches(0.85),
             d, size=8.5, color=GRAY, spacing=1.22)
    ...
    killers = [
        ...
        ("자금 설계사", "공고 나열이 아니라 조달 설계",
         "\"5천만원이 필요한데 어떻게 만들죠?\" —\n실공고 900건에서 주는 돈 → 보증·이자지원 →\n융자 순으로 쌓고 월 상환액까지. 전액 시중\n대출 대비 \"○○만원 아낍니다\"를 숫자로 —\n기준 금리는 '가정' 명시, 전 항목 원문 링크."),
    ]
    ...
    text(s, Inches(0.75), Inches(7.0), Inches(11.8), Inches(0.35),
         "그 밖의 결: 두 동네 비교(A vs B 요인 승부표) · 찜 D-day + RPA 원문 순찰 · 마감 캘린더(.ics) · "
         "내 가게 원클릭 브리핑 · 상담 리포트 인쇄 · 음성 대화 · 금소법 검사기",
         size=10.5, color=GRAY)

    # ── 7.7 실제 화면 II — 금융 AI의 결 ─────────────────────────────────
    s = blank(prs)
    header(s, "SCREENS Ⅱ", "과정은 글라스박스로, 가치는 원 단위로")
    text(s, Inches(0.75), Inches(1.9), Inches(11.8), Inches(0.4),
         "AI가 어떻게 답했는지(실측 로그), 설계가 얼마를 아끼는지(이자 절감), "
         "권리가 언제 사라지는지(D-day) — 전부 실서비스 화면입니다.",
         size=13, color=GRAY)
    shots2 = [("glassbox3.png", Inches(0.75), Inches(3.9), Inches(2.8),
               "글라스박스 — 실측 로그"),
              ("plan_bench.png", Inches(3.75), Inches(2.75), Inches(3.9),
               "자금 설계 — 시중 대출 대비 절감액"),
              ("goldentime.png", Inches(8.35), Inches(2.15), Inches(4.2),
               "골든타임 — 권리 마감 D-day")]
    for fn, x, hh, cw, cap in shots2:
        fp = os.path.join(SHOTS, fn)
        if os.path.exists(fp):
            s.shapes.add_picture(fp, x, Inches(2.5), height=hh)
            text(s, x, Inches(6.55), cw, Inches(0.35),
                 cap, size=11, bold=True, color=BROWN)
