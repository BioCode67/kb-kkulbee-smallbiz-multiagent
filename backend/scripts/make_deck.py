...
def build() -> str:
    ...
    # ── 7.5 자유주제 3종 — "어떻게 했지" ─────────────────────────────────
    s = blank(prs)
    header(s, "BEYOND", "데이터사이언스 · RPA · 생성 · 접근성 — 자유 기능 여섯")
    cols6 = [
        ("두 동네 비교", "A vs B를 같은 자로",
         "\"연남동이랑 성수동 비교해줘\" 한 마디로\n우세 판정 + 요인 6개 승부표 + 생활인구 대조.\n후보 둘 사이의 고민을 화면 하나로."),
        ("RPA 원문 순찰", "색인은 스냅샷, 공고는 살아 움직인다",
         "찜한 공고 전부를 기계가 원문에서 재확인 —\n마감 연장·조기 마감·서식 교체 감지.\n신청서식 hwp 즉시 다운로드, 마감은 .ics로."),
        ("민원서 초안 생성", "규칙이 법조문, LLM은 사실관계만",
         "\"수수료를 왜 떼요\" 한 문장이 제출 가능한\n민원서 꼴이 됩니다. 없는 날짜·금액은\n[빈칸] 강제 — §를 지어낼 수 없는 구조."),
        ("닮은 상권 찾기", "업종 구성 247차원 코사인",
         "연남동 ↔ 광안리·서교동·망원동.\n붙어 있는 동네가 아니라 '성격이 닮은' 동네 —\n2호점 후보의 출발점. 질의 0.4ms."),
        ("음성 대화 + 지도 투어", "브라우저 내장 · 다운로드 0",
         "말로 묻고 꿀비가 읽어 줍니다. 지도는 전국에서\n그 동네로 날아들어 4막 투어(점포→열지도→비교).\n금융 접근성 — 이 대회가 상을 준 각도."),
        ("계산기 두 대", "융자의 다음 질문은 '한 달에 얼마?'",
         "매칭 공고의 한도·금리가 자동으로 채워지는\n상환 시뮬레이터(원리금·원금·만기일시)와\n손익분기 계산기. 확약 아님을 화면에 명시."),
    ]
    for i, (t, sub, d) in enumerate(cols6):
        x = Inches(0.75 + (i % 3) * 4.02)
        y0 = Inches(2.0 + (i // 3) * 2.62)
        rect(s, x, y0, Inches(3.72), Inches(2.42),
             fill=RGBColor(0xFA, 0xF8, 0xF4), line=LINE)
        text(s, x + Inches(0.25), y0 + Inches(0.18), Inches(3.25), Inches(0.4),
             t, size=14.5, bold=True)
        text(s, x + Inches(0.25), y0 + Inches(0.58), Inches(3.25), Inches(0.35),
             sub, size=10.5, bold=True, color=RGBColor(0x9A, 0x6B, 0x00))
    ...
    killers = [
        ...
        ("자금 설계사", "공고 나열이 아니라 조달 설계",
         "\"5천만원이 필요한데 어떻게 만들죠?\" —\n실공고 900건에서 주는 돈 → 보증·이자지원 →\n융자 순으로 쌓아 갚을 돈을 최소화하고, 융자분은\n월 상환액까지. 금액 근거 없는 공고는 스택에\n넣지 않고 '검토 후보'로만 — 전 항목 원문 링크."),
    ]
